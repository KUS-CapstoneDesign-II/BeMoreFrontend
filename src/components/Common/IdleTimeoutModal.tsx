    ...
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="활동 없음 경고">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[92%] sm:w-[420px] bg-white dark:bg-gray-900 rounded-xl shadow-soft-lg p-5 sm:p-6">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onEnd} className="px-3 py-2 rounded-lg border text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-700">종료</button>
          <button onClick={onContinue} className="px-3 py-2 rounded-lg text-sm bg-primary-600 hover:bg-primary-700 text-white">계속</button>
        </div>
      </div>
    </div>
  );
